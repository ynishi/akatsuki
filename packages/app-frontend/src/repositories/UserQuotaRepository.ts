import { supabase } from '../lib/supabase'

/**
 * Database record types for user_quotas table
 */
export type PlanType = 'free' | 'pro' | 'enterprise'

export interface UserQuotaDatabaseRecord {
  id: string
  user_id: string
  plan_type: PlanType
  monthly_request_limit: number
  current_month: string // YYYY-MM format
  requests_used: number
  created_at: string
  updated_at: string
}

export interface UserCurrentUsageView {
  user_id: string
  plan_type: PlanType
  monthly_request_limit: number
  requests_used: number
  remaining_requests: number
  current_month: string
}

export interface QuotaAvailability {
  hasQuota: boolean
  remaining: number
  limit: number
}

export interface UserWithQuota extends UserQuotaDatabaseRecord {
  user: {
    id: string
    email: string
  }
}

/**
 * UserQuota Repository
 * user_quotas table data access layer
 *
 * Manages monthly LLM API usage quotas per user
 *
 * リポジトリパターンの利点:
 * - データアクセスロジックを一箇所に集約
 * - テストが容易（モックしやすい）
 * - Supabaseクライアントの実装詳細を隠蔽
 */
export class UserQuotaRepository {
  /**
   * Get current month's quota for a user
   */
  static async findCurrentMonthQuota(userId: string): Promise<UserQuotaDatabaseRecord | null> {
    const currentMonth = new Date().toISOString().slice(0, 7) // YYYY-MM

    const { data, error } = await supabase
      .from('user_quotas')
      .select('*')
      .eq('user_id', userId)
      .eq('current_month', currentMonth)
      .single()

    if (error) {
      if (error.code === 'PGRST116') {
        // No record found - return null (will be auto-created on first API call)
        return null
      }
      console.error('Quota fetch error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }

  /**
   * Get quota for a specific month
   */
  static async findByMonth(userId: string, month: string): Promise<UserQuotaDatabaseRecord | null> {
    const { data, error } = await supabase
      .from('user_quotas')
      .select('*')
      .eq('user_id', userId)
      .eq('current_month', month)
      .single()

    if (error) {
      if (error.code === 'PGRST116') {
        return null
      }
      console.error('Quota fetch error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }

  /**
   * Get all quotas for a user (history)
   */
  static async findAllByUserId(userId: string, limit = 12): Promise<UserQuotaDatabaseRecord[]> {
    const { data, error } = await supabase
      .from('user_quotas')
      .select('*')
      .eq('user_id', userId)
      .order('current_month', { ascending: false })
      .limit(limit)

    if (error) {
      console.error('Quota history fetch error:', error)
      throw error
    }

    return (data || []) as UserQuotaDatabaseRecord[]
  }

  /**
   * Get current usage view (using user_current_usage view)
   */
  static async getCurrentUsage(userId: string): Promise<UserCurrentUsageView | null> {
    const { data, error } = await supabase
      .from('user_current_usage')
      .select('*')
      .eq('user_id', userId)
      .single()

    if (error) {
      if (error.code === 'PGRST116') {
        return null
      }
      console.error('Current usage fetch error:', error)
      throw error
    }

    return data as UserCurrentUsageView
  }

  /**
   * Check if user has remaining quota
   */
  static async checkQuotaAvailability(userId: string): Promise<QuotaAvailability> {
    const usage = await this.getCurrentUsage(userId)

    if (!usage) {
      // No quota record - will be created on first API call
      return {
        hasQuota: true,
        remaining: 100, // default free plan
        limit: 100,
      }
    }

    return {
      hasQuota: usage.remaining_requests > 0,
      remaining: usage.remaining_requests,
      limit: usage.monthly_request_limit,
    }
  }

  /**
   * Update plan type
   * Note: This should typically be done via backend/admin, not client-side
   */
  static async updatePlanType(userId: string, planType: PlanType): Promise<UserQuotaDatabaseRecord> {
    const currentMonth = new Date().toISOString().slice(0, 7)

    const { data, error } = await supabase
      .from('user_quotas')
      .update({ plan_type: planType })
      .eq('user_id', userId)
      .eq('current_month', currentMonth)
      .select()
      .single()

    if (error) {
      console.error('Plan update error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }

  // ========== Admin Methods ==========

  /**
   * Get all users with their current quota (Admin only)
   */
  static async getAllUsersWithQuota(): Promise<UserWithQuota[]> {
    const currentMonth = new Date().toISOString().slice(0, 7)

    // RPC関数を使ってauth.usersのemailを含むquota情報を取得
    const { data, error } = await supabase.rpc('get_users_with_quotas', { target_month: currentMonth })

    if (error) {
      console.error('Get users with quotas error:', error)
      throw error
    }

    // データを整形（userオブジェクトにemailを含める）
    return (data || []).map(
      (row: {
        quota_id: string
        user_id: string
        plan_type: PlanType
        monthly_request_limit: number
        current_month: string
        requests_used: number
        created_at: string
        updated_at: string
        email: string
      }) => ({
        id: row.quota_id,
        user_id: row.user_id,
        plan_type: row.plan_type,
        monthly_request_limit: row.monthly_request_limit,
        current_month: row.current_month,
        requests_used: row.requests_used,
        created_at: row.created_at,
        updated_at: row.updated_at,
        user: {
          id: row.user_id,
          email: row.email,
        },
      })
    )
  }

  /**
   * Update quota limit (Admin only)
   */
  static async updateQuotaLimit(userId: string, newLimit: number): Promise<UserQuotaDatabaseRecord> {
    const currentMonth = new Date().toISOString().slice(0, 7)

    const { data, error } = await supabase
      .from('user_quotas')
      .update({ monthly_request_limit: newLimit })
      .eq('user_id', userId)
      .eq('current_month', currentMonth)
      .select()
      .single()

    if (error) {
      console.error('Update quota limit error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }

  /**
   * Reset usage count for a user (Admin only)
   */
  static async resetUsageCount(userId: string): Promise<UserQuotaDatabaseRecord> {
    const currentMonth = new Date().toISOString().slice(0, 7)

    const { data, error } = await supabase
      .from('user_quotas')
      .update({ requests_used: 0 })
      .eq('user_id', userId)
      .eq('current_month', currentMonth)
      .select()
      .single()

    if (error) {
      console.error('Reset usage error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }

  /**
   * Create or update quota for a user (Admin only)
   */
  static async upsertQuota(
    userId: string,
    quotaData: Partial<Omit<UserQuotaDatabaseRecord, 'id' | 'user_id' | 'current_month' | 'created_at' | 'updated_at'>>
  ): Promise<UserQuotaDatabaseRecord> {
    const currentMonth = new Date().toISOString().slice(0, 7)

    const { data, error } = await supabase
      .from('user_quotas')
      .upsert(
        {
          user_id: userId,
          current_month: currentMonth,
          ...quotaData,
        },
        {
          onConflict: 'user_id,current_month',
        }
      )
      .select()
      .single()

    if (error) {
      console.error('Upsert quota error:', error)
      throw error
    }

    return data as UserQuotaDatabaseRecord
  }
}
